...
// Non-linear PRGA with updated keystream logic
void enhancedPRGA(unsigned char* S, unsigned char* data, int len) {
  int i = 0, j = 0;
  Serial.print("Keystream: ");
  for (int k = 0; k < len; k++) {
    i = (i + 1) % SBOX_SIZE;
    j = (j + (S[i] ^ (3 * i + 7))) % SBOX_SIZE;
    std::swap(S[i], S[j]);
    int t = ((S[i] ^ S[j]) + (i ^ j)) % SBOX_SIZE;
    byte ks = (S[t] ^ ((S[i] & S[j]) | (i & j))) % SBOX_SIZE;
    Serial.print(ks, HEX);
    Serial.print(" ");
    
    // Apply non-linear encryption step
    data[k] = ((data[k] ^ ks) + ((data[k] & ks) | (data[k] >> 2))) % 256;
  }
  Serial.println();
}
